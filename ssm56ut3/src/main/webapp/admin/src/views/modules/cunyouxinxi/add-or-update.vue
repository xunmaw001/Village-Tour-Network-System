...
export default {
  ...
  methods: {
	...
	this.ruleForm.jingdianjieshao = this.ruleForm.jingdianjieshao.replace(reg,'../../../ssm56ut3/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `cunyouxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.cunyouxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
