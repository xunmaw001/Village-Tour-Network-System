...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussmeishitechanCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussmeishitechanCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussmeishitechan');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussmeishitechan');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussmeishitechan', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussmeishitechan', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussmeishitechan/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussmeishitechan', this.detail);
				this.$utils.msg('审核成功');
